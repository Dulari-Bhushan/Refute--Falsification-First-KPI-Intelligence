...
> **Picking this up in a new session?** The prototype is **complete** — every item on the original build plan (§7 here, and the more detailed [PLAN.md](PLAN.md)) is built, validated, and committed. [GAPS.md](GAPS.md) is a separate, later audit against the brief's 10 named "real-world complexities" specifically (a different, more granular checklist than §6/§7 below) — all 7 findings it flagged are closed as of this update; there is no known open item on either list. If you're continuing work, it's refinement (new scenarios, a real data source, letting real feedback accrue toward the 30-outcome calibration threshold), not gap-closing. Read §0 (this section) for what exists and where; §6 for the objective-by-objective coverage table; [PLAN.md](PLAN.md)'s top "STATUS UPDATE" block for the most recent session's specific changes. Run `uv run python run_pipeline.py` then `uv run uvicorn api.main:app --reload` to see it working before changing anything.
...
| Layer | File | Status |
|---|---|---|
| Semantic contract | [semantic/kpi_contract.yaml](semantic/kpi_contract.yaml) | 4 KPIs (each tagged with a `domain`: sales/marketing/hr), 5 sources, **4** entitlement roles (row + column + domain-scoped — see `marketing_analyst`), maturity rule, lineage, an `operational_constraints.max_accounts_per_rep` capacity ceiling |
| Synthetic data | [data/generate_synthetic_data.py](data/generate_synthetic_data.py) | 9,145 POS rows, 3 sources at 3 cadences, 1 true cause + 5 decoys (including a planted external-event/weather-storm cluster), sparse-history category, definition-drift pair |
| Reconciliation | [data/reconciliation.py](data/reconciliation.py) | Multi-cadence resampling, freshness tracking, **two** independently-kinded falsifiable claims: cross-source revenue agreement (correctly flags the planted 2% gross/net definition drift) and rep-attributed-revenue-within-region-revenue logical bounds (correctly holds, max ratio 13.0%); `compute_missing_data_rates()` quantifying what each cross-source join actually drops (10% pos↔finance, 2.5% pos↔CRM); `compute_data_quality_scores()` grading each source HIGH/MEDIUM/LOW/n/a from real coverage-completeness + system-of-record signals, not a binary flag |
...
| L4 Compiler | [engine/l4_compiler.py](engine/l4_compiler.py) | Pydantic predicate schema with hard-validated `refutes_if`, whitelisted parameterized SQL compiler, compile-time entitlement checks (row + column + **domain**, `check_domain_entitlement()`), `sql_hash()` traceability attached to every fetched panel; all **four** spec archetypes now have a real fixture (placebo/specificity/precedence/**dose_response**) |
| L5 Adjudicate | [engine/l5_adjudicate.py](engine/l5_adjudicate.py) | DiD (log-scale, unit fixed effects, cluster-robust → HC1 fallback below 4 clusters/side), mandatory parallel-trends check, the power gate, BH-FDR correction, every verdict carries its treatment/control SQL + hash through to the ledger; `evaluate_dose_response_test()` adjudicates the marketing-spend hypothesis via Spearman rank correlation with its own MDE-equivalent power gate |
| L6 Narrate + Ledger | [engine/l6_narrate_ledger.py](engine/l6_narrate_ledger.py) | Persona rendering (3 roles, entitlement-filtered), SQLite ledger with SQL-traceability columns, telemetry, feedback-as-falsification-event, `detect_contradictory_verdicts()` distinguishing same-evidence re-narration from genuine independent contradiction, a persisted `entitlement_checks` audit log, and `determine_delivery_channel()`/`simulate_delivery()` for urgency-gated (honestly-simulated) delivery-channel routing |
| L6 Action recommendation | [engine/action_recommendation.py](engine/action_recommendation.py) | Investigation-aware, not West-only: works for any investigation in `engine/investigations.py`. Gathers that investigation's SURVIVED verdict(s), L1 signal context, and real operational-capacity data for whichever dimension the surviving hypothesis was tested on (`crm_headcount.csv` for `rep_id`, the new `data/synthetic/fulfillment_center_ops.csv` for `fulfillment_center`), then has an LLM synthesize a grounded root-cause-and-action writeup — or, with no LLM backend selected or a failed call, falls back to a deterministic composition of the same real evidence. Explicitly flags (`inferred_without_operational_data`, capped confidence) when no operational dataset exists yet for a dimension (e.g. `product_category`), rather than inventing a feasibility check |
| L4 Live LLM generation | [engine/l4_llm_generation.py](engine/l4_llm_generation.py) | Local GPU (Qwen2.5-3B-Instruct via `outlines`, token-level schema-constrained decoding) or OpenRouter, two-gate validation (schema + semantic domain), graceful fallback, real telemetry, a ledger-backed predicate cache keyed on topic-cluster identity (verified end-to-end: a repeat run skips the model load entirely). Accepted, adjudicated live-generated verdicts are persisted into `data/synthetic/l5_verdicts.json` (tagged `source="llm_generated"`, collision-safe id renaming) so they actually appear in the main hypothesis grid and Narrated Brief — not just the one-time run results panel — with identical statistical trust to hand-authored hypotheses, marked with a visible "LLM-generated" provenance badge in the UI |
...
| Knowledge graph | [engine/knowledge_graph.py](engine/knowledge_graph.py) | A real, dependency-free directed graph built from the semantic contract + this run's actual verdicts (~35 nodes, ~55 edges) — `related()`, `blast_radius()`, `shared_mechanism()` answer questions the flat YAML couldn't, rendered in the UI as a draggable/zoomable D3 force graph where clicking a node highlights it and its direct neighbors/edges (dimming the rest) and clicking again or clicking empty canvas clears it, alongside a dropdown+search query panel |
...
| `h_weather_disruption` | precedence | **KILLED** | Same pattern, a genuinely different driver class: a planted regional-storm ticket cluster whose own changepoint (week 34) also comes after the KPI's — the "external events" complexity from GAPS.md item 1, closed the same rigorous way as the internal decoys |
...
| `h_marketing_spend_cut` | dose_response | **INCONCLUSIVE** | Spearman rho=-0.15, p=0.65 — not significant, and with only 12 (region, channel) strata the test could only reliably detect \|rho\|≥0.73 — genuinely underpowered, not swept into KILLED |
...
This is the exact three-way split (obvious decoys killed / underpowered decoys inconclusive / true cause survives) that §8's evaluation targets call the most important thing to get right — and it's now real, not aspirational. The marketing hypothesis landing on INCONCLUSIVE rather than a clean KILLED is itself a real result, not a shortfall: it shows the power gate applies uniformly across every archetype, including the one (`dose_response`) that had never actually been exercised before this pass — see [GAPS.md](GAPS.md).
...
| 6 | Recommend practical actions grounded in business levers, constraints, decision rights | ✅ | `driver → controllable lever → action → expected impact → owner → confidence → monitoring plan`, populated from real L1/L5 numbers, generated per-investigation (not just West) by [engine/action_recommendation.py](engine/action_recommendation.py); checked against a real operational constraint for whichever dimension the surviving hypothesis names (`max_accounts_per_rep` for rep-attrition, backlog-vs-surge-capacity for a fulfillment-center cause) and qualified/escalated when it doesn't fully fit — a recommendation that ignores whether the team has room to execute it isn't actually actionable. When no operational dataset exists yet for a dimension, the recommendation says so explicitly rather than fabricating a feasibility number |
