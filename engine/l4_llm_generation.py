...
def topic_cache_key(topic: TopicCandidate, kpi_context: dict) -> str:
    ...
    model_identity = config["openrouter_model"] if config["backend"] == "openrouter" else ("none" if config["backend"] == "none" else MODEL_NAME)
    payload = json.dumps(
        {
...
def _generate_and_validate(system_prompt: str, user_prompt: str) -> GenerationResult:
    ...
    if backend == "none":
        return GenerationResult(
            predicate_dict=None,
            accepted=False,
            reason="No-model mode selected -- by design, no LLM call is made. Only the templated/rule-based fixture path runs.",
            prompt_tokens=0,
            completion_tokens=0,
            latency_ms=0.0,
            raw_output="",
...
def main(region: str = "West") -> None:
    ...
    config = get_llm_config()
    backend = config["backend"]
    model_identity = config["openrouter_model"] if backend == "openrouter" else ("none" if backend == "none" else MODEL_NAME)
    ...
    if backend == "none":
        print(f"Backend: no model (rule-based only) -- no model loaded, no network call made. Every topic reports not-generated by design; {n_cached}/{len(candidates)} topic(s) had a prior-run cache entry (not used while this backend is selected).\n")
    elif backend == "local":
    ...
    for topic, result in results:
        with telemetry_span(
            ...
            is_llm_call=backend != "none" and not result.cache_hit,  # a cache hit made no LLM call this run, and "none" backend never calls a model at all -- the telemetry's LLM-vs-deterministic breakdown should say so honestly, not count either as a fresh call
            model=model_identity,
