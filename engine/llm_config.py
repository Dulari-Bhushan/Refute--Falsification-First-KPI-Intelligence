...
Three backends:
  ...
  - "none": no model is loaded and no network call is made, period. Every
    candidate topic is reported as not-generated by design (not a failure)
    and the dashboard falls back entirely to the templated/rule-based
    fixture path. This exists so a run can be forced fully deterministic --
    "nothing leaves this machine, nothing is read by a model" -- for
    demoing or auditing the non-LLM path without touching hardware or a
    key at all.
...
VALID_BACKENDS = {"local", "openrouter", "none"}
